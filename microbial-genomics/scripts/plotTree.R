...
#info.ordered<-info[tip.label.order,]
}
...
# PLOT TREE
par(mar=rep(0,4))
tlp<-plot.phylo(tl,no.margin=T,show.tip.label=tip.labels,label.offset=offset,edge.width=lwd,edge.color=edge.color,xaxs="i", yaxs="i", y.lim=c(0.5,length(tl$tip)+0.5),cex=tipLabelSize,font=2)
...
if ((!is.null(outputPDF) | !is.null(outputPNG)) & closeDev==TRUE) {
dev.off()
}
